...
class MenuScanCradle extends StatefulWidget {
  MenuScanCradle({super.key, required this.title, this.cradleId});
  String title;
  int? cradleId;

  @override
  State<MenuScanCradle> createState() => _MenuScanCradleState();
}

class _MenuScanCradleState extends State<MenuScanCradle> {
  final MobileScannerController cameraController = MobileScannerController();
  final AudioPlayer _audioPlayer = AudioPlayer();
                          ...
                          if (code != null && code.isNotEmpty) {
                            ...
                            if (!context
                              ...
                              context
                                  ...
                                  .addScannedData(code, 'QR');
                              _playBeep();
                              await provider.scanCradle(
                                  context, code, widget.cradleId!);
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                    content: Text('Scan\nBerhasil: $code')),
                              );
